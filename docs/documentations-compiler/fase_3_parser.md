...
- `src/parser.py`: Engine *Recursive Descent* Parser lengkap yang menangani Presedensi Operator Matematika, Blok Kode, Percabangan (`lamun`, `lamun_deui`, `sanesna`), Perulangan, dan Pemanggilan Fungsi.

## Hasil Validasi
Berdasarkan metrik pengujian di `tests/test_parser.py`:
1. **Tingkat Kelulusan:** 100% *Pass* pada seluruh kasus uji coba kompleks.
2. **Waktu Respons:** Total waktu eksekusi validasi memakan waktu di bawah **0.01 detik**.
