...
Nilai konstanta murni seperti `Bener` (True), `Bohong` (False), dan `Euweuh` (None) diproses secara natif sebagai `KEYWORD` agar relevan dengan arsitektur dasar Python 3.

## Hasil Validasi
1. **Kecepatan Eksekusi:** Seluruh *test suites* yang berjalan di dalam `tests/test_lexer.py` sukses diselesaikan dalam hitungan **milidetik**.
