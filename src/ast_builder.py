...
class ASTBuilder:
    ...
    def generic_build(self, node: ParseNode) -> ASTNode:
        raise NotImplementedError(f"Euweuh metode build pikeun ParseNode '{node.name}'")
    ...
    def build_function_def_stmt(self, node: ParseNode) -> FunctionDef:
        ...
        for param_token in params_node.children:
            if isinstance(param_token, Token) and (param_token.type.name == 'IDENTIFIER' or param_token.value == 'urang'):
    ...
    def build_try_stmt(self, node: ParseNode) -> TryStmt:
        ...
        while idx < len(node.children) and isinstance(node.children[idx], Token) and node.children[idx].value == 'kacuali':
            ...
            if isinstance(node.children[idx], Token) and node.children[idx].type.name == 'IDENTIFIER':
                ...
                idx += 1
                if isinstance(node.children[idx], Token) and node.children[idx].value == 'minangka':
        ...
        if idx < len(node.children) and isinstance(node.children[idx], Token) and node.children[idx].value == 'ahirna':
            idx += 2  # skip ahirna {
            finally_block = []
            while idx < len(node.children) and isinstance(node.children[idx], ParseNode):
    ...
    def build_terus_weh_stmt(self, node: ParseNode) -> ContinueStmt: return ContinueStmt()
    ...
    build_lemparkeun_stmt = _build_unary_ctrl
    build_pastikeun_stmt = _build_unary_ctrl
    build_hapus_stmt = _build_unary_ctrl
    build_kabeh_stmt = _build_unary_ctrl
    build_lain_lokal_stmt = _build_unary_ctrl
    ...
    def build_primary(self, node: ParseNode) -> ASTNode:
        ...
        if token.type.name == 'NUMBER':
            ...
            elif val == 'Bohong': return Literal(False)
            elif val == 'Euweuh': return Literal(None)
            elif val == 'urang': return Identifier('urang')
            elif val == 'kolot': return Identifier('kolot')
        
        raise Exception(f"Token primary teu dipikanyaho: {token}")
