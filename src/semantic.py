...
class SymbolTable:
    def __init__(self):
        ...
        builtins = [
            'nyarita', 'panjang', 'himpunan', 'mutlak', 'Error', 
            'baca', 'buka', 'sleep', 'daptar', 'jenis', 'tanya',
    ...
    def pop_scope(self):
        if len(self.scopes) > 1:
            ...
            raise Exception("Teu bisa ngahapus global scope")
...
class SemanticAnalyzer:
    ...
    def visit_Identifier(self, node: Identifier):
        if node.name in ['urang', 'kolot']:
            return  # Special keywords for class scope
            
        if not self.symbol_table.lookup(node.name):
            raise SemanticError(f"Variabel atawa fungsi '{node.name}' can acan dideklarasikeun.")
    ...
    def visit_BreakStmt(self, node: BreakStmt):
        if self.loop_depth == 0:
            raise SemanticError("Sintaksis 'eureun' (break) aya di luar perulangan.")

    def visit_ContinueStmt(self, node: ContinueStmt):
        if self.loop_depth == 0:
            raise SemanticError("Sintaksis 'terus_weh' (continue) aya di luar perulangan.")
